"""Conformance suite mapped to the profile checklist rows.
...
REQUIRES_HUMAN_EVIDENCE = {
    ...
    "EN-06": "open EN 18223 deviation decision: field mapping, an approval by a different named individual, and a dated decision",
...
def test_en02_identifier_uniqueness_and_non_reuse(client, bound_dpp):
    ...
    bound = client.get(f"/v1/dpps/{bound_dpp}", headers=HEADERS["brand"]).json()
    already_bound = bound["carriers"][0]["encodedIdentifier"]
    clash = client.post(
        f"/v1/dpps/{second}/carriers",
        json={
            "carrierType": "uhfRfid",
            "encodingScheme": "gs1Sgtin96",
            "encodedIdentifier": already_bound,
...
def test_api02_json_xml_rdf_mapping_covers_mandatory_fields():
    package = json.loads((SPEC_DIR / "mappings" / "dpp-mapping-1.0.0.json").read_text(encoding="utf-8"))
    mapped = {row["jsonPath"] for row in package["rows"]}
...
def test_dat10_disagreement_stays_discoverable(client, dpp_id):
    for observation_id, percentage, method, at in (
        ("urn:sort4circ:obs:lab", 95, "labQuantitativeIso1833", "2026-08-10T09:14:01.902Z"),
        ("urn:sort4circ:obs:nir", 80, "nirSpectroscopy", "2026-08-11T09:14:01.902Z"),
    ):
        client.post(
            f"/v1/dpps/{dpp_id}/observations",
            headers=HEADERS["pssrSystem"],
...
def test_synthetic_profile_integrity_roundtrip():
    """A generated record validates, digests, anchors and verifies end to end."""
    from sort4circ_dpp.canonical import digest
    from sort4circ_dpp.ledger.memory import InMemoryLedger

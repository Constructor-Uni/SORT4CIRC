...
def test_mapping_covers_every_mandatory_leaf_field():
    """The normative mapping package covers every mandatory leaf the profile requires."""
    package = json.loads(
        (ROOT / "spec" / "mappings" / "dpp-mapping-1.0.0.json").read_text(encoding="utf-8")
    )
    paths = {row["jsonPath"] for row in package["rows"]}
    required = {
...
def test_api_negotiates_complete_dpp_xml(client):
    response = client.post(
        ...
        headers={
            "X-DPP-Role": "brand",
            "X-DPP-Organisation": "urn:example:org:manufacturer-a",

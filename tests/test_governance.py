...
def environmental_record() -> dict:
    return {
        ...
        "preparationDate": "2026-08-01", "preparedBy": person("cu-a", "preparer"),
...
def test_environmental_record_requires_separation_of_duties():
    ...
    assert any("different named individual" in error for error in validate_environmental_selection(record))
...
def test_en18223_closed_record_requires_different_approver_and_closure_evidence():
    ...
    template.update({
        ...
        "preparedBy": {"individualId": "cu-a", "name": "cu-a", "role": "preparer"},
        "recommendation": "synthetic test recommendation", "status": "closed",
        "approvedBy": {"individualId": "cu-a", "name": "cu-a", "role": "approver"},
        "decisionDate": "2026-08-03", "decisionDeadline": "2026-12-31",
        "closureEvidence": ["synthetic test artefact"],
        "notification": {"recipient": "standards liaison", "notifiedAt": "2026-08-04"},
    })
    assert any("different named individual" in error for error in validate_en18223_deviation(template))

...
| `tools/anchor_bench.py` | anchoring latency, time to confirmation, fee and metered energy, per ledger platform | a machine you control, with credentials and network access to the platforms under test |
...
Workload: Python 3.11, one uvicorn process, an in-memory store and in-memory ledger
adapter. 200 seeded synthetic passports, 2000 requests per class per level. Latency in
milliseconds, measured at the client.

The host's CPU model, core count and memory are deliberately not recorded here. Absolute
figures depend on the machine, so treat the numbers below as the shape of the behaviour
(relative cost per class, and how it degrades with concurrency) rather than as a
specification. Re-run the harness on your own hardware to get figures you can rely on.
...
with one caller and 53.72 ms at p99 with eight concurrent callers, against 920 ms. The
